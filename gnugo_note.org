...
龍的每一個相關聯的眼位空間都提供一個本地博弈，它產生一個本地博弈樹。這個本地博弈
的得分是它產生眼位的數量。通常如果玩家輪流做出最佳選擇，最終的得分將相差 0 或 1。
在這種情況下，本地博弈可以用一個整數或半整數來表示。因此如果 ='n(O)'= 是 ='O'=
...
本地博弈。遊戲的結果以 =:max,min= 的形式列在眼形之後， 其中 =max= 是當 ='O'= 先
...
我們可以如下概括本地博弈：兩名玩家 ='O'= 和 ='X'= 輪流落子，或者 pass。
...
我們沒有試圖去說明本地博弈的終止狀態（有些可能是共活）或者評分。

** 一個例子
這是一個本地博弈，無論誰先落子，都只會產生一隻眼：
